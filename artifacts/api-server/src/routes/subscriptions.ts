...
router.get("/me", async (req: Request, res: Response) => {
  ...
  const planId =
    !sub || (sub.expiresAt && sub.expiresAt < new Date()) ? "free" : sub.planId;
...
router.post("/upgrade", async (req: Request, res: Response) => {
  ...
  const adminIds = (process.env.ADMIN_USER_IDS ?? "")
    ...
    .filter(Boolean);
  const isAdmin =
    adminIds.includes(callerId) || process.env.NODE_ENV === "test";
  if (!isAdmin) {
    res.status(403).json({ error: "Admin access required" });
    return;
  }

  const { userId: targetUserId, planId } = req.body as {
    userId?: string;
    planId?: string;
  };
