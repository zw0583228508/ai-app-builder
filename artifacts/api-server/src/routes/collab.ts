...
export function broadcastProjectUpdate(projectId: string, payload: object) {
  ...
  const msg = JSON.stringify({ type: "project-updated", projectId, ...payload });
...
function broadcastPresence(projectId: string, sender?: CollabClient) {
  ...
  const cursors = Array.from(room)
    .filter(c => c.cursor)
    .map(c => ({ clientId: c.clientId, color: c.color, name: c.name, ...c.cursor }));
  const msg = JSON.stringify({ type: "presence", projectId, viewerCount: count, cursors });
  for (const client of room) {
    if (client.ws.readyState === 1) client.ws.send(msg);
  }
}

export async function handleCollabWs(ws: WebSocket, req: import("http").IncomingMessage) {
  ...
  const sidFromBearer = authHeader?.startsWith("Bearer ") ? authHeader.slice(7) : undefined;
  ...
  const [proj] = await db
    ...
    .from(projectsTable)
    .where(and(eq(projectsTable.id, numericId), isNull(projectsTable.deletedAt)));
  ...
  const client: CollabClient = { ws, projectId, clientId, color, name, joinedAt: Date.now() };
  room.add(client);

  logger.info({ projectId, viewers: room.size }, "Collab client joined");

  // Send current viewer count + cursors to new joiner
  const cursors = Array.from(room).filter(c => c.cursor && c !== client)
    .map(c => ({ clientId: c.clientId, color: c.color, name: c.name, ...c.cursor }));
  ws.send(JSON.stringify({ type: "presence", projectId, viewerCount: room.size, cursors, yourClientId: clientId, yourColor: color }));
  ...
  ws.on("message", (data) => {
    try {
      const msg = JSON.parse(data.toString()) as { type: string; line?: number; col?: number; file?: string };
      if (msg.type === "ping") {
        ws.send(JSON.stringify({ type: "pong" }));
      } else if (msg.type === "cursor") {
        client.cursor = { line: msg.line ?? 0, col: msg.col ?? 0, file: msg.file ?? "" };
        // Broadcast cursor to all others in room
        const cursorMsg = JSON.stringify({ type: "cursor", clientId, color, name, line: client.cursor.line, col: client.cursor.col, file: client.cursor.file });
        for (const other of room) {
          if (other !== client && other.ws.readyState === 1) other.ws.send(cursorMsg);
        }
      }
    } catch {
      // ignore malformed
