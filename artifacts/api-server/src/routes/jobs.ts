...
router.get("/", async (req: Request, res: Response) => {
  ...
  if (projectId)
    conditions.push(eq(jobQueueTable.projectId, Number(projectId)));
...
router.post("/:id/cancel", async (req: Request, res: Response) => {
  ...
  const [job] = await db
    ...
    .where(
      and(eq(jobQueueTable.id, jobId), eq(jobQueueTable.status, "pending")),
    )
    .returning();

  if (!job) {
    res.status(400).json({ error: "Job cannot be cancelled (not pending)" });
    return;
  }
  res.json({ job });
});
...
router.get("/stats/summary", async (_req: Request, res: Response) => {
  ...
  const deadLetterCount = await db
    ...
    .from(jobQueueTable)
    .where(
      and(eq(jobQueueTable.status, "failed"), sql`attempts >= max_attempts`),
    );
    ...
    await db
      ...
      .set({
        ...
        runAfter: shouldRetry
          ? new Date(Date.now() + Math.pow(2, job.attempts ?? 0) * 1000)
          : null,
        updatedAt: new Date(),
      })
      .where(eq(jobQueueTable.id, jobId));
