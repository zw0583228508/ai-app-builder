...
router.post("/", async (req: Request, res: Response) => {
  ...
  const { appName, description } = req.body as {
    appName?: string;
    description?: string;
  };
  ...
  sendEvent({
    type: "text",
    content: `🏗️ יוצר SaaS מלא: **${appName}**...\n\n`,
  });

  const fullPrompt = SAAS_TEMPLATE(
    appName,
    description ?? "A complete SaaS platform",
    project.stack ?? "react",
  );
...
Start directly with <!DOCTYPE html> — no preamble or explanation.`,
    ...
    if (!clientDisconnected) {
      ...
      const rawBlock = fullCode.includes("<!DOCTYPE")
        ? fullCode.substring(fullCode.indexOf("<!DOCTYPE"))
        : fullCode;
    ...
    sendEvent({
      error: e instanceof Error ? e.message : "SaaS generation failed",
    });
    res.end();
  }
});
