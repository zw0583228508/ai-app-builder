...
router.get("/", async (req: Request, res: Response) => {
  ...
  const tokenRows = await db
    ...
    .where(
      and(
        eq(usageLogsTable.projectId, projectId),
  ...
  const records = await db
    ...
    .where(
      and(
        eq(costRecordsTable.projectId, projectId),
        gte(costRecordsTable.createdAt, since),
      ),
    )
...
router.post("/record", async (req: Request, res: Response) => {
  ...
  if (!resourceType || quantity === undefined) {
    res.status(400).json({ error: "resourceType and quantity required" });
    return;
  }
...
Return ONLY valid JSON.`;
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: e instanceof Error ? e.message : "Cost optimization failed",
      });
  }
});
