  ...
  try {
    ...
    const [saved] = await db
      ...
      .values({
        ...
        dbSchema: (plan.db_schema ?? []) as unknown as Record<
          string,
          unknown
        >[],
        integrations: (plan.integrations ?? []) as unknown as Record<
          string,
          unknown
        >[],
        ...
        agentAssignments: (plan.agent_assignments ?? {}) as Record<
          string,
          unknown
        >,
      })
    ...
    res
      .status(500)
      .json({ error: e instanceof Error ? e.message : "Planner failed" });
  }
});
