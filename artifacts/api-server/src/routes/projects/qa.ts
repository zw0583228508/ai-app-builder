...
Return ONLY valid JSON.`;
  ...
  try {
    ...
    const qa = JSON.parse(raw.replace(/```json\n?|\n?```/g, "").trim()) as Record<string, unknown>;
    ...
    const [saved] = await db.insert(qaTestResultsTable).values({
      projectId,
      testSuite: testSuite as unknown as Record<string, unknown>[],
      passed: 0,
      failed: 0,
      skipped: testSuite.length,
      coveragePercent: Number((qa.coverage_analysis as Record<string, unknown>)?.estimated_coverage_percent ?? 0),
      autoFixSuggestions: fixes as unknown as Record<string, unknown>[],
    }).returning();

    res.json({ qa: { ...qa, id: saved.id, summary } });
  } catch (e: unknown) {
    res.status(500).json({ error: e instanceof Error ? e.message : "QA generation failed" });
  }
});
