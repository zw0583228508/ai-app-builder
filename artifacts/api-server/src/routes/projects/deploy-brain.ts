  ...
  try {
    ...
    const [saved] = await db
      ...
      .values({
        ...
        alternativeOptions: (brain.alternative_options ??
          []) as unknown as Record<string, unknown>[],
        decisionFactors: (brain.decision_factors ?? {}) as Record<
          string,
          unknown
        >,
      })
    ...
    res
      .status(500)
      .json({ error: e instanceof Error ? e.message : "Deploy Brain failed" });
  }
});
