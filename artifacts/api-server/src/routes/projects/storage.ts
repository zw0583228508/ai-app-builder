...
router.post("/upload-url", async (req: Request, res: Response) => {
  ...
  const { name } = req.body as { name?: string; size?: number; contentType?: string };
...
router.post("/", async (req: Request, res: Response) => {
  ...
  if (!name || !objectPath) { res.status(400).json({ error: "name and objectPath are required" }); return; }
...
router.delete("/:objectId", async (req: Request, res: Response) => {
  ...
  if (!projectId || !objectId) { res.status(400).json({ error: "Invalid ids" }); return; }

  const [obj] = await db
    .select()
    .from(projectStorageObjectsTable)
    .where(and(eq(projectStorageObjectsTable.id, objectId), eq(projectStorageObjectsTable.projectId, projectId)));

  if (!obj) { res.status(404).json({ error: "Object not found" }); return; }

  await db.delete(projectStorageObjectsTable).where(eq(projectStorageObjectsTable.id, objectId));
