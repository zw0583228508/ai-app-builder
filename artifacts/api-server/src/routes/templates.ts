...
import { db, templatesTable, templateFilesTable, projectsTable, projectFilesTable } from "@workspace/db";
...
router.get("/my", async (req: Request, res: Response) => {
  ...
  const templates = await db.select().from(templatesTable).where(eq(templatesTable.userId, userId)).orderBy(desc(templatesTable.createdAt));
  res.json({ templates });
});
...
router.post("/", async (req: Request, res: Response) => {
  ...
  const { projectId, title, description, isPublic, tags } = req.body as {
    projectId?: number; title?: string; description?: string; isPublic?: boolean; tags?: string[];
  };
  ...
  if (files.length > 0) {
    await db.insert(templateFilesTable).values(
      files.map(f => ({ templateId: template.id, path: f.path, content: f.content, language: f.language, isEntrypoint: f.isEntrypoint }))
    );
  }
...
router.post("/:id/use", async (req: Request, res: Response) => {
  ...
  if (!template.isPublic && template.userId !== userId) {
    res.status(403).json({ error: "Template is private" }); return;
  }

  // Bump use count
  await db.update(templatesTable).set({ uses: sql`${templatesTable.uses} + 1` }).where(eq(templatesTable.id, templateId));
  ...
  // Copy template files
  const files = await db.select().from(templateFilesTable).where(eq(templateFilesTable.templateId, templateId));
  if (files.length > 0) {
    await db.insert(projectFilesTable).values(
      files.map(f => ({ projectId: project.id, path: f.path, content: f.content, language: f.language, isEntrypoint: f.isEntrypoint }))
    );
  }
...
router.delete("/:id", async (req: Request, res: Response) => {
  ...
  const [template] = await db.select().from(templatesTable).where(and(eq(templatesTable.id, templateId), eq(templatesTable.userId, userId)));
  if (!template) { res.status(404).json({ error: "Template not found" }); return; }
