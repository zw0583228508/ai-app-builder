...
router.post("/join", async (req: Request, res: Response) => {
  ...
  const [team] = await db
    .select()
    .from(teamsTable)
    .where(eq(teamsTable.inviteCode, inviteCode.trim()));
  if (!team) {
  ...
  await db
    .insert(teamMembersTable)
    .values({ teamId: team.id, userId, role: "editor" });
...
router.post("/:id/invite-email", async (req: Request, res: Response) => {
  ...
  if (team.ownerId !== userId) {
    res.status(403).json({ error: "Only team owner can invite" });
    return;
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: e instanceof Error ? e.message : "Email failed" });
  }
});
