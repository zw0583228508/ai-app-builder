...
router.get("/:projectId", async (req: Request, res: Response) => {
  ...
  const isIdle = env.lastActiveAt && (Date.now() - new Date(env.lastActiveAt).getTime()) > 30 * 60 * 1000;
...
router.post("/:projectId/start", async (req: Request, res: Response) => {
  ...
  if (!env) {
    const [updated] = await db
      .update(runtimeEnvironmentsTable)
      .set({ status: "running", ...usage, lastActiveAt: new Date(), autoShutdownAt, updatedAt: new Date() })
      .where(eq(runtimeEnvironmentsTable.projectId, projectId))
      .returning();
...
router.post("/:projectId/stop", async (req: Request, res: Response) => {
  ...
  const [updated] = await db
    .update(runtimeEnvironmentsTable)
    .set({ status: "stopped", cpuUsage: 0, ramUsageMb: 0, updatedAt: new Date() })
    .where(eq(runtimeEnvironmentsTable.projectId, projectId))
    .returning();
