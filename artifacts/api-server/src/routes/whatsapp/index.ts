...
import { db, projectsTable, projectDnaTable, projectMessagesTable, analyticsEventsTable } from "@workspace/db";
...
async function handleWhatsAppMessage(
  ...
  if (
    ...
    try {
      ...
      const deployRes = await fetch(`http://localhost:${process.env.PORT ?? 8080}/api/projects/${projectId}/deploy`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider: "netlify", projectId }),
      });
      if (deployRes.ok) {
        const deployData = await deployRes.json() as { url?: string; deployUrl?: string };
  ...
  // ── CSS color change ──
  const colorMatch = text.match(/שנה צבע ל([^\s]+)/i) || text.match(/change color to ([^\s]+)/i);
  if (colorMatch?.[1]) {
...
async function startSession(projectId: number) {
  ...
  sessions.set(projectId, { sock: null, qr: null, status: "connecting", jid: null });
...
router.post("/:projectId/connect", async (req, res) => {
  ...
  const s = sessions.get(projectId);
  res.json({ status: s?.status ?? "connecting", qr: s?.qr ?? null, jid: s?.jid ?? null });
});
