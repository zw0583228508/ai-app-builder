...
export async function createProject(data: {
  ...
  const [project] = await db
    ...
    .values({
      ...
      type: data.type,
      stack: data.stack,
      userMode: (data.userMode ?? "entrepreneur") as "entrepreneur" | "builder" | "developer" | "maker",
      userId: data.userId,
    })
    .returning();
  return project;
...
export async function updateProjectMode(id: number, userMode: string) {
  const [updated] = await db
    .update(projectsTable)
    .set({ userMode: userMode as "entrepreneur" | "builder" | "developer" | "maker", updatedAt: new Date() })
    .where(eq(projectsTable.id, id))
    .returning();
  return updated;
}
