...
describe("detectChatIntent", () => {
  describe("deploy intent", () => {
    it("detects 'deploy' keyword", () => {
      expect(detectChatIntent("deploy the app now", false).intent).toBe(
        "deploy",
      );
    });
    ...
    it("detects netlify keyword", () => {
      expect(detectChatIntent("publish to netlify", false).intent).toBe(
        "deploy",
      );
    });
  });
  ...
  describe("fix intent", () => {
    ...
    it("detects 'error' keyword", () => {
      expect(
        detectChatIntent("there is an error in the form", true).intent,
      ).toBe("fix");
    });
    ...
    it("detects 'not working' variant", () => {
      expect(detectChatIntent("the login doesn't work", true).intent).toBe(
        "fix",
      );
    });
  });
  ...
  describe("inspect intent", () => {
    it("detects 'what is' with existing code", () => {
      expect(detectChatIntent("what is in this project", true).intent).toBe(
        "inspect",
      );
    });
  ...
  describe("add_feature intent", () => {
    it("detects 'add' with existing code", () => {
      expect(detectChatIntent("add a login button", true).intent).toBe(
        "add_feature",
      );
    });
    ...
    it("detects continue keywords with existing code", () => {
      expect(detectChatIntent("continue building", true).intent).toBe(
        "add_feature",
      );
  ...
  describe("edit intent", () => {
    it("detects 'change' with existing code", () => {
      expect(detectChatIntent("change the color to blue", true).intent).toBe(
        "edit",
      );
    });
    it("detects 'update' with existing code", () => {
      expect(detectChatIntent("update the title text", true).intent).toBe(
        "edit",
      );
    });
  ...
  describe("create intent", () => {
    it("triggers create when no existing code", () => {
      expect(detectChatIntent("build me a landing page", false).intent).toBe(
        "create",
      );
    });
    it("triggers create on 'from scratch' even with existing code", () => {
      expect(detectChatIntent("rebuild from scratch", true).intent).toBe(
        "create",
      );
    });
    it("triggers create on 'start over'", () => {
      expect(
        detectChatIntent("start over with a new design", true).intent,
      ).toBe("create");
    });
  ...
  describe("priority ordering", () => {
    it("deploy takes priority over edit keywords", () => {
      expect(
        detectChatIntent("deploy and update the colors", true).intent,
      ).toBe("deploy");
    });
    ...
    it("git_push takes priority over fix", () => {
      expect(detectChatIntent("push to github fix commit", true).intent).toBe(
        "git_push",
      );
    });
  });
