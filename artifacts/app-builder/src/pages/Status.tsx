  ...
  useEffect(() => {
    fetch(`${BASE}/api/healthz`)
      ...
      .then((data: { status: string; uptime: number; timestamp: string }) => {
        setApi({ status: data.status === "ok" ? "ok" : "error", uptime: data.uptime, timestamp: data.timestamp });
      })
  ...
  const services = [
    {
      ...
      status: api.status,
      detail: api.status === "ok" ? `זמן פעולה: ${formatUptime(api.uptime ?? 0)}` : api.error,
    },
    {
      name: "ממשק משתמש",
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white flex flex-col items-center py-16 px-4" dir="rtl">
      ...
      >
        ...
        <div className={`rounded-2xl border p-5 mb-8 flex items-center gap-4 ${
          ...
          <div>
            <p className="text-lg font-semibold">
              {api.status === "loading" ? "בודק מצב..." : api.status === "ok" ? "כל המערכות פועלות תקין" : "יש בעיות במערכת"}
            </p>
            {api.timestamp && (
              <p className="text-sm text-white/40">
                עדכון אחרון: {new Date(api.timestamp).toLocaleTimeString("he-IL")}
              </p>
            )}
          </div>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs ${
                  svc.status === "ok" ? "text-green-400" : svc.status === "loading" ? "text-white/40" : "text-red-400"
                }`}>
                  {svc.detail}
                </span>
              </div>
        ...
        <p className="text-center text-xs text-white/20 mt-8">
          לדיווח על בעיה:{" "}
          <a href="mailto:support@example.com" className="text-cyan-400/60 hover:text-cyan-400">
            support@example.com
          </a>
        </p>
      </motion.div>
