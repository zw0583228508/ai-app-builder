...
export default function Pricing() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white flex flex-col items-center py-20 px-4" dir="rtl">
      <motion.div
        ...
        {plans.map((plan, i) => (
          <motion.div
            ...
            className={`relative rounded-2xl border p-7 flex flex-col ${plan.color} ${
              plan.highlight ? "bg-cyan-500/5 shadow-lg shadow-cyan-500/10" : "bg-white/[0.02]"
            }`}
          >
            {plan.highlight && (
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                ...
                <plan.icon className={`w-5 h-5 ${plan.highlight ? "text-cyan-400" : "text-white/60"}`} />
              </div>
              <div>
            ...
            <button
              onClick={() => {
                if (plan.href && !plan.href.startsWith("mailto:")) navigate(plan.href);
