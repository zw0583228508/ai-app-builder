...
import { Loader2, Search, Layers, Globe, Lock, Zap, Star, Code2, ArrowRight, BookmarkPlus } from "lucide-react";
...
export function Gallery() {
  ...
  const useMutation_ = useMutation({
    mutationFn: (id: number) => apiFetch<{ project: { id: number } }>(`/api/templates/${id}/use`, { method: "POST" }),
  ...
  const filtered = unique.filter(t => {
    ...
    const matchSearch = !q || t.title.toLowerCase().includes(q) || (t.description ?? "").toLowerCase().includes(q) || t.tags.some(g => g.toLowerCase().includes(q));
    const matchStack = stackFilter === "all" || t.stack === stackFilter;
    return matchSearch && matchStack;
  });

  return (
    <div className="min-h-screen bg-background" dir="rtl" style={{ fontFamily: HE }}>
      {/* Header */}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {loadingPublic ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map(t => (
              <div key={t.id} className="group bg-slate-900/60 border border-white/8 rounded-xl overflow-hidden hover:border-cyan-500/30 transition-all">
                {/* Preview placeholder */}
                <div className="h-32 bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center relative">
                  ...
                  <div className={cn("absolute top-2 right-2 text-[10px] px-1.5 py-0.5 rounded border font-mono", STACK_COLORS[t.stack] ?? "text-slate-400 bg-slate-800 border-slate-700")}>
                    {t.stack}
                  </div>
                  {!t.isPublic && (
                ...
                <div className="p-3 space-y-2">
                  <div>
                    ...
                    {t.description && <p className="text-xs text-slate-400 line-clamp-2 mt-0.5">{t.description}</p>}
                  </div>

                  {t.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {t.tags.slice(0, 3).map(tag => (
                        <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 border border-white/5">{tag}</span>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    >
                      {useMutation_.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <ArrowRight className="w-3 h-3" />}
                      השתמש
                    </button>
                  </div>
                </div>
