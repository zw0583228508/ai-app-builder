...
export default function Home() {
  ...
  const STARTER_PROMPTS = [
    {
      icon: ShoppingBag,
      label: isRTL ? "חנות אונליין" : "Online store",
      prompt:
        "Build a beautiful e-commerce store with product gallery, cart and checkout.",
    },
    {
      icon: Globe,
      label: isRTL ? "דף נחיתה SaaS" : "SaaS landing page",
      prompt:
        "Build a stunning SaaS landing page with hero, features and pricing.",
    },
    {
      icon: Briefcase,
      label: isRTL ? "אתר עסקי" : "Business site",
      prompt:
        "Build a professional business website with services, reviews and contact.",
    },
    {
      icon: BarChart3,
      label: isRTL ? "דשבורד אנליטיקס" : "Analytics dashboard",
      prompt: "Build an analytics dashboard with charts, KPIs and data tables.",
    },
    {
      icon: BookOpen,
      label: isRTL ? "פורטפוליו" : "Portfolio",
      prompt: "Build a stunning portfolio with gallery, bio and contact form.",
    },
    {
      icon: Palette,
      label: isRTL ? "אתר יצירתי" : "Creative site",
      prompt: "Build a creative website with animations and unique design.",
    },
    {
      icon: Rocket,
      label: isRTL ? "אפליקציית SaaS" : "SaaS app",
      prompt: "Build a full SaaS app with auth, dashboard and billing.",
    },
    {
      icon: Database,
      label: isRTL ? "אפליקציית CRUD" : "CRUD app",
      prompt: "Build an app with database, forms and CRUD operations.",
    },
  ];
  ...
  const currentModeObj =
    USER_MODES.find((m) => m.id === selectedMode) || USER_MODES[0];
        ...
        <div className="relative z-10 flex flex-col items-center min-h-full px-4 pt-10 pb-20">
            ...
            <h1 className="text-3xl sm:text-4xl font-black text-white leading-tight mb-3 tracking-tight">
              {isRTL ? (
                <>
                  בנה אפליקציה מלאה
                  <br />
                  <span style={{ color: PRIMARY }}>עם AI</span>
                </>
              ) : (
                <>
                  Build a full app
                  <br />
                  <span style={{ color: PRIMARY }}>with AI</span>
                </>
              )}
            </h1>
          ...
          {!projectsLoading && recentProjects.length > 0 && (
            ...
            >
              ...
              >
                ...
                <ChevronRight
                  className={cn(
                    "w-3.5 h-3.5 text-white/15 group-hover:text-[hsl(191,90%,42%)]/60 transition-all shrink-0",
                    isRTL
          ...
          >
            <div className="relative rounded-2xl border border-white/10 bg-white/[0.04] shadow-2xl shadow-black/50 focus-within:border-[hsl(191,90%,42%)]/50 focus-within:shadow-[hsl(191,90%,42%)]/10 focus-within:shadow-2xl transition-all duration-300">
              ...
              <div
                className={cn(
                  "flex items-center justify-between px-4 pb-4 pt-1 gap-3",
                  isRTL && "flex-row-reverse",
                )}
              >
            ...
            {sessionExpired && (
              <div
                className="mt-2.5 flex flex-col gap-2 px-3 py-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-[12px] text-amber-300 animate-in slide-in-from-top-1 duration-200"
                dir="rtl"
            ...
            {createError && (
              <div
                className="mt-2.5 flex items-center gap-2 px-3 py-2 bg-red-500/10 border border-red-500/25 rounded-xl text-[12px] text-red-400 animate-in slide-in-from-top-1 duration-200"
                dir={isRTL ? "rtl" : "ltr"}
                style={{
                  ...
                  : recentProjects.map((p, i) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-medium text-white/65 group-hover:text-white/90 truncate transition-colors">
                            {p.title}
                          </p>
                          <p className="text-[11px] text-white/25" dir="ltr">
                            {format(new Date(p.updatedAt), "d MMM, HH:mm")}
                          </p>
                        </div>
          ...
          >
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-1.5">
              {[
                ...
                {
                  icon: "🌐",
                  label: isRTL ? "פרסום בלחיצה" : "1-click deploy",
                },
