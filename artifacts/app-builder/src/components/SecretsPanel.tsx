...
export function SecretsPanel({ projectId }: SecretsPanelProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (vars: { key: string; value: string; environment: string }) =>
      apiFetch(`/api/projects/${projectId}/secrets`, {
        method: "POST",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      apiFetch(`/api/projects/${projectId}/secrets/${id}`, {
        method: "DELETE",
      }),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["project-secrets", projectId] }),
        ...
        <div className="mx-4 mt-3 p-3 bg-card border border-border/60 rounded-xl flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={
                !newKey.trim() || !newValue.trim() || createMutation.isPending
              }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {isLoading ? (
          ...
          filtered.map((s) => (
            <div
              key={s.id}
              className="flex items-center gap-2 p-3 bg-card border border-border/40 rounded-xl group"
            >
