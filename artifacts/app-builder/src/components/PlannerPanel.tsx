      ...
      <div className="px-4 py-3 border-b border-white/10 bg-gradient-to-r from-[hsl(191,90%,42%)]/10 to-transparent">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">
            Planner Agent — מתכנן AI
          </h2>
        </div>
        <p className="text-xs text-white/40 mt-0.5">
          הזן רעיון ו-AI ייצר תכנית מקיפה: features, screens, APIs, DB
        </p>
      </div>
        ...
        {!plan && !loading && (
          <div className="flex flex-col items-center justify-center h-48 gap-3">
            ...
            <p className="text-white/30 text-sm text-center">
              הזן רעיון ולחץ "צור תכנית" לקבלת
              <br />
              תכנית מקצועית מלאה
            </p>
          </div>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                {
                  label: "Features",
                  val: plan.features?.length ?? 0,
                  icon: <Zap className="w-3 h-3" />,
                  color: "text-cyan-400",
                },
                {
                  label: "Screens",
                  val: plan.screens?.length ?? 0,
                  icon: <Layout className="w-3 h-3" />,
                  color: "text-purple-400",
                },
                {
                  label: "APIs",
                  val: plan.apis?.length ?? 0,
                  icon: <Server className="w-3 h-3" />,
                  color: "text-yellow-400",
                },
                {
                  label: "שעות",
                  val: Math.round(plan.estimated_hours ?? 0),
            ...
            {[
                    ...
                    {plan.screens?.map((s, i) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <code className="text-[9px] text-purple-300 bg-purple-500/10 px-1 rounded">
                            {s.route}
                          </code>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {s.components?.map((c, j) => (
                            <span
                              key={j}
                              className="text-[9px] bg-white/5 text-white/50 px-1.5 py-0.5 rounded border border-white/10"
                            >
                              {c}
                            </span>
                          ))}
              ...
              },
              {
                ...
                count: plan.apis?.length,
                content: (
                  <div className="space-y-1.5 pt-2">
                    {plan.apis?.map((a, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 bg-white/5 rounded p-2 border border-white/10"
                      >
                        <span
                          className={cn(
                        ...
                        <code className="text-[11px] text-white/70 flex-1">
                          {a.endpoint}
                        </code>
                        {a.auth && (
                  ...
                  <div className="space-y-2 pt-2">
                    {plan.db_schema?.map((t, i) => (
                      <div
                        ...
                        className="bg-white/5 rounded-lg p-2.5 border border-white/10"
                      >
                        <div className="text-xs font-semibold text-blue-300 mb-1.5">
                          📊 {t.table}
                        </div>
                        <div className="space-y-0.5">
                          {t.columns?.map((c, j) => (
                            <div key={j} className="flex items-center gap-2">
                              <span className="text-[10px] text-white/70 w-28">
                                {c.name}
                              </span>
                              <code className="text-[9px] text-blue-300/70">
                                {c.type}
                              </code>
                            </div>
                          ))}
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] bg-white/10 text-white/50 px-1.5 rounded">
                      {section.count}
                    </span>
                  </div>
