...
export function QaPanel({ projectId }: QaPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<"tests" | "fixes" | "coverage">(
    "tests",
  );
  ...
  const qualityLabel = (score: number) =>
    score >= 80 ? "איכות גבוהה" : score >= 60 ? "בינוני" : "דרוש שיפור";
      ...
      <div className="px-4 py-3 border-b border-white/10 bg-gradient-to-r from-green-500/10 to-transparent">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">
            AI QA System — בדיקות אוטומטיות
          </h2>
        </div>
        <p className="text-xs text-white/40 mt-0.5">
          מייצר tests אוטומטיים ומזהה בעיות בקוד
        </p>
      </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-white/5 rounded-lg p-2.5 border border-white/10 text-center">
                <div
                  className={cn(
                    "text-2xl font-bold",
                    qualityColor(result.qa_summary?.quality_score ?? 0),
                  )}
                >
              ...
              <div className="bg-white/5 rounded-lg p-2.5 border border-white/10 text-center">
                <div className="text-2xl font-bold text-blue-400">
                  {Math.round(
                    result.coverage_analysis?.estimated_coverage_percent ?? 0,
                  )}
                  %
                </div>
              ...
              <div className="bg-white/5 rounded-lg p-2.5 border border-white/10 text-center">
                <div className="text-2xl font-bold text-white">
                  {result.test_suite?.length ?? 0}
                </div>
                <div className="text-[10px] text-white/40">Tests</div>
                <div className="text-[10px] text-red-400">
                  {result.qa_summary?.critical_tests ?? 0} critical
                </div>
              </div>
            </div>
            ...
            <div className="flex border-b border-white/10">
              {(["tests", "fixes", "coverage"] as const).map((tab) => (
                ...
                >
                  {tab === "tests"
                    ? `Tests (${result.test_suite?.length ?? 0})`
                    : tab === "fixes"
                      ? `Fixes (${result.auto_fix_suggestions?.length ?? 0})`
                      : "Coverage"}
                </button>
              ))}
            </div>
              ...
              <div className="space-y-2">
                {result.test_suite?.map((test, i) => (
                  ...
                  >
                    <button
                      onClick={() =>
                        setOpenTest(openTest === test.id ? null : test.id)
                      }
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          PRIORITY_DOT[test.priority] ?? "bg-gray-500",
                        )}
                      />
                ...
                {result.auto_fix_suggestions?.map((fix, i) => (
                  <div
                    ...
                    className={cn(
                      ...
                      SEV_COLORS[fix.severity] ?? "border-white/10 bg-white/5",
                    )}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span className="text-xs font-semibold">{fix.issue}</span>
                      <span className="text-[9px] px-1.5 py-0.5 rounded border mr-auto">
                        {fix.severity}
                      </span>
                    </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                  <div className="flex justify-between text-xs mb-1.5">
                    ...
                    <span className="text-white font-semibold">
                      {Math.round(
                        result.coverage_analysis?.estimated_coverage_percent ??
                          0,
                      )}
                      %
                    </span>
                  ...
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${result.coverage_analysis?.estimated_coverage_percent ?? 0}%`,
                      }}
                    />
                  </div>
                </div>
                {result.coverage_analysis?.risk_areas?.length > 0 && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                    ...
                    {result.coverage_analysis.risk_areas.map((area, i) => (
                ...
                {result.qa_summary?.recommendations?.length > 0 && (
                  <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                    ...
                    {result.qa_summary.recommendations.map((r, i) => (
                      <p key={i} className="text-xs text-white/50 mb-1">
                        • {r}
                      </p>
                    ))}
                  </div>
