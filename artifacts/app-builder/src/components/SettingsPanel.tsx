...
import {
  Settings, Trash2, AlertTriangle, Save, Globe, Code2, RefreshCw, Copy, Check,
} from "lucide-react";
...
export default function SettingsPanel({ projectId, project }: SettingsPanelProps) {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-5" style={{ fontFamily: HE }} dir="rtl">
      ...
      <div className="bg-white/5 rounded-xl border border-white/10 p-4 space-y-4">
        <h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider">פרטים בסיסיים</h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            >
              {PROJECT_TYPES.map((t) => (
                <option key={t.id} value={t.id} className="bg-[#1a1d27]">{t.label}</option>
              ))}
            </select>
          </div>
          <div>
            <label className="text-xs text-white/50 block mb-1">
              <Code2 className="w-3 h-3 inline ml-1" />סטאק
            </label>
            <select
              value={stack}
            ...
            >
              {STACKS.map((s) => (
                <option key={s.id} value={s.id} className="bg-[#1a1d27]">{s.label}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      <div className="bg-red-500/5 rounded-xl border border-red-500/20 p-4 space-y-3">
        ...
        {!showDeleteConfirm ? (
          ...
          <div className="space-y-2">
            <p className="text-xs text-red-400">האם אתה בטוח? הפרויקט יועבר לפח ויימחק בעוד 30 יום.</p>
            <div className="flex gap-2">
              <button
