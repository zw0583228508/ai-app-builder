...
export function parseHtml(html: string) {
  ...
  const hasCharts =
    ...
    html.includes("apex");
  const hasAnimations =
    html.includes("framer") || html.includes("gsap") || html.includes("anime");
...
export function analyzeSeo(html: string) {
  ...
  results.push({
    ...
    suggestion:
      titleLen === 0
        ? "חסר תג title"
        : titleLen < 30
          ? "קצר מדי (מינ׳ 30 תווים)"
          : titleLen > 65
            ? "ארוך מדי (מקס׳ 65 תווים)"
            : undefined,
  });
  ...
  results.push({
    ...
    suggestion:
      descLen === 0
        ...
        : descLen < 120
          ? "קצר מדי (מינ׳ 120 תווים)"
          : descLen > 160
            ? "ארוך מדי (מקס׳ 160 תווים)"
            : undefined,
  });
  ...
  results.push({
    ...
    suggestion: !ogTitle
      ? "חסרים תגי og:title, og:description, og:image"
      : undefined,
  });
  ...
  results.push({
    key: "canonical",
    label: "Canonical URL",
    value: canonicalMatch?.[1] || null,
    status: canonicalMatch ? "ok" : "warn",
    suggestion: !canonicalMatch ? "מומלץ להוסיף תג canonical" : undefined,
  });
  ...
  results.push({
    key: "viewport",
    label: "Viewport meta",
    value: viewportMatch ? "קיים" : null,
    status: viewportMatch ? "ok" : "error",
    suggestion: !viewportMatch
      ? "חסר viewport meta — האתר לא ידידותי למובייל"
      : undefined,
  });

  const images = [...html.matchAll(/<img[^>]*>/gi)];
  const imagesWithoutAlt = images.filter(
    (m) => !m[0].includes("alt=") || m[0].match(/alt=["']\s*["']/),
  );
  results.push({
    key: "imgalt",
    label: `Alt לתמונות (${images.length})`,
    value:
      imagesWithoutAlt.length === 0
        ? "✓ כל התמונות"
        : `${imagesWithoutAlt.length} חסרות`,
    status:
      imagesWithoutAlt.length === 0
        ? "ok"
        : imagesWithoutAlt.length > 3
          ? "error"
          : "warn",
    suggestion:
      imagesWithoutAlt.length > 0
        ? `${imagesWithoutAlt.length} תמונות ללא alt text`
        : undefined,
  });

  const structuredData =
    html.includes('"@context"') && html.includes('"@type"');
  results.push({
    key: "schema",
    label: "Structured Data",
    value: structuredData ? "JSON-LD נמצא" : null,
    status: structuredData ? "ok" : "warn",
    suggestion: !structuredData ? "מומלץ להוסיף JSON-LD Schema" : undefined,
  });

  const score = Math.round(
    (results.filter((r) => r.status === "ok").length / results.length) * 100,
  );
  ...
  const items = [
    {
      ...
      status: size < 50000 ? "ok" : size < 150000 ? "warn" : ("error" as any),
      detail: size < 50000 ? "מצוין" : size < 150000 ? "בסדר" : "גדול מדי",
    },
    {
      label: "סקריפטים חיצוניים",
      value: `${extJs}`,
      status: extJs <= 3 ? "ok" : extJs <= 6 ? "warn" : ("error" as any),
      detail: extJs <= 3 ? "מצוין" : "שקול לאחד",
    },
    {
      label: "CSS חיצוני",
      value: `${extCss}`,
      status: extCss <= 2 ? "ok" : ("warn" as any),
      detail: extCss <= 2 ? "בסדר" : "שקול לאחד",
    },
    {
      label: "JS מוטמע",
      value: formatSize(inlineJs),
      status:
        inlineJs < 30000 ? "ok" : inlineJs < 80000 ? "warn" : ("error" as any),
      detail: "",
    },
    {
      label: "CSS מוטמע",
      value: formatSize(inlineCss),
      status: inlineCss < 20000 ? "ok" : ("warn" as any),
      detail: "",
    },
    {
      label: "תמונות",
      value: `${imgs} (${lazyImgs} lazy)`,
      status:
        imgs === 0 || lazyImgs === imgs
          ? "ok"
          : lazyImgs > 0
            ? "warn"
            : ("error" as any),
      detail: imgs > 0 && lazyImgs === 0 ? "הוסף loading='lazy'" : "",
    },
    {
      label: "async/defer לסקריפטים",
      value: hasAsync ? "✓ יש" : "✗ חסר",
      status: hasAsync ? "ok" : ("warn" as any),
      detail: !hasAsync ? "הוסף async/defer" : "",
    },
    {
      label: "Preload hints",
      value: hasPreload ? "✓ יש" : "לא נמצא",
      status: hasPreload ? "ok" : ("warn" as any),
      detail: "",
    },
    {
      label: "iframe",
      value: `${iframes}`,
      status: iframes === 0 ? "ok" : ("warn" as any),
      detail: iframes > 0 ? "iframes מאטים טעינה" : "",
    },
  ];
...
export function analyzeA11y(html: string) {
  ...
  const noAlt = imgs.filter(
    (m) => !m[0].includes("alt=") || /alt=["']\s*["']/.test(m[0]),
  );
  if (noAlt.length > 0)
    issues.push({
      severity: "error",
      message: `${noAlt.length} תמונות ללא alt text`,
      count: noAlt.length,
    });
  ...
  const noLabel = inputs.filter((m) => {
    ...
    if (!id)
      return !m[0].includes("aria-label") && !m[0].includes("placeholder");
    return !html.includes(`for="${id}"`) && !html.includes(`for='${id}'`);
  });
  if (noLabel.length > 0)
    issues.push({
      severity: "error",
      message: `${noLabel.length} שדות קלט ללא label`,
      count: noLabel.length,
    });

  const buttons = [...html.matchAll(/<button[^>]*>([\s\S]*?)<\/button>/gi)];
  const emptyBtns = buttons.filter(
    (m) => !m[1].replace(/<[^>]*>/g, "").trim() && !m[0].includes("aria-label"),
  );
  if (emptyBtns.length > 0)
    issues.push({
      severity: "error",
      message: `${emptyBtns.length} כפתורים ללא טקסט/aria-label`,
  ...
  if (emptyLinks.length > 0)
    issues.push({
      severity: "warn",
      message: `${emptyLinks.length} קישורים ריקים`,
  ...
  if (!hasLang)
    issues.push({
      severity: "error",
      message: "חסר lang attribute בתג <html>",
    });
  ...
  if (!hasSkipLink)
    issues.push({
      severity: "info",
      message: "מומלץ להוסיף skip navigation link",
    });
  ...
  if (!hasAriaLandmarks)
    issues.push({
      severity: "warn",
      message: "חסרים ARIA landmarks (main, nav, header)",
    });
  ...
  if (hasHighTabIndex)
    issues.push({
      severity: "warn",
      message: "tabindex גבוה מ-1 פוגע בסדר הניווט",
    });
  ...
  if (!hasContrast)
    issues.push({
      severity: "info",
      message: "בדוק ניגודיות צבעים (WCAG 2.1 AA)",
    });
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-white/3 rounded-lg border border-white/8 group hover:bg-white/6 transition-colors">
      ...
      <div className="flex items-center gap-1 shrink-0">
        <button
          onClick={copy}
          className="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-white/10 text-muted-foreground hover:text-foreground transition-all"
          title="העתק תג"
        >
          {copied ? (
        ...
        <a
          ...
          className="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-white/10 text-muted-foreground hover:text-primary transition-all"
        >
    ...
    <code>
      {lines.map((line, i) => (
        <span key={i} className="flex">
          <span className="w-10 shrink-0 text-muted-foreground/30 select-none text-right pr-4">
            {i + 1}
          </span>
          <span
            className="flex-1"
...
export function highlightLine(line: string, lang: string): string {
  ...
  if (lang === "javascript") {
    const keywords =
      /\b(const|let|var|function|return|if|else|for|while|class|import|export|default|async|await|new|this|typeof|null|undefined|true|false|of|in|from)\b/g;
    return esc(line)
      .replace(
        /(\/\/.*)/g,
        '<span style="color:#6b7280;font-style:italic">$1</span>',
  ...
  if (lang === "html") {
    return esc(line)
      ...
      .replace(
        /([\w-]+=)(&quot;|")?([^&"<>]*)/g,
        '<span style="color:#60a5fa">$1</span><span style="color:#86efac">"$3"</span>',
      )
      .replace(
        /((&lt;!--[\s\S]*?--&gt;))/g,
        '<span style="color:#6b7280;font-style:italic">$1</span>',
