...
export function WhatsAppPanel({ projectId }: WhatsAppPanelProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      const r = await fetch(`/api/whatsapp/${projectId}/connect`, {
        method: "POST",
      });
      const data = (await r.json()) as {
        status: ConnectionStatus;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          className={`flex items-center gap-1.5 text-xs ${statusConfig[status].color}`}
        >
          <StatusIcon
            size={13}
        ...
        {status === "disconnected" && (
          <>
            <p className="text-xs text-slate-400 leading-relaxed">
              חבר את ה-WhatsApp שלך ושלוט על הפרויקט ישירות מהסמארטפון — שאל
              שאלות, קבל סטטיסטיקות, ואפילו פרסם.
            </p>
            <button
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 rounded-lg bg-green-500/10 border border-green-500/20">
              ...
              <div className="flex-1 min-w-0">
                ...
                {jid && (
                  <p
                    className="text-[11px] text-green-400/70 mt-0.5 truncate"
                    dir="ltr"
                  >
                    {jid.split(":")[0]}
                  </p>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1.5">
          {FEATURES.map(({ cmd, desc }) => (
            <div
              key={cmd}
              className="flex items-center gap-2 p-2 rounded-lg bg-slate-800/30 border border-slate-700/40"
            >
      ...
      <p className="text-[10px] text-slate-600 leading-relaxed">
        כל ההודעות מטופלות בעברית. ה-AI עונה על שאלות על הפרויקט ומפעיל פעולות
        בהתאם לפקודות.
      </p>
    </div>
  );
