...
import { Activity, Play, Square, RefreshCw, Loader2, Cpu, MemoryStick, Zap, Clock, Globe, AlertCircle } from "lucide-react";
import { cn } from "@/lib/utils";

interface RuntimePanelProps { projectId: number; }
...
const STATUS_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  running: { label: "פועל", color: "text-green-400", dot: "bg-green-400 animate-pulse" },
  idle: { label: "ממתין", color: "text-yellow-400", dot: "bg-yellow-400" },
  stopped: { label: "מכובה", color: "text-white/40", dot: "bg-white/30" },
  creating: { label: "נוצר...", color: "text-blue-400", dot: "bg-blue-400 animate-pulse" },
...
export function RuntimePanel({ projectId }: RuntimePanelProps) {
  ...
  const fetchEnv = async () => {
    try {
      const res = await fetch(`/api/runtime/${projectId}`, { credentials: "include" });
  ...
  const doAction = async (action: "start" | "stop" | "restart") => {
    ...
    try {
      const res = await fetch(`/api/runtime/${projectId}/${action}`, { method: "POST", credentials: "include" });
      if (!res.ok) return;
      const text = await res.text();
      if (!text) return;
    ...
    <div className="h-full flex flex-col bg-[hsl(220,16%,6%)] overflow-hidden" dir="rtl" style={{ fontFamily: "'Rubik', sans-serif" }}>
      ...
      <div className="px-4 py-3 border-b border-white/10 bg-gradient-to-r from-blue-500/10 to-transparent">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">Runtime Control Plane</h2>
        </div>
        <p className="text-xs text-white/40 mt-0.5">ניהול סביבת ריצה — CPU, RAM, GPU, Auto-Shutdown</p>
      </div>
          ...
          <>
              ...
              {isRunning && env && (
                <div className="grid grid-cols-3 gap-3 mt-2">
                  <div className="bg-white/5 rounded-lg p-2.5 text-center">
                    ...
                    <div className="text-lg font-bold text-white">{(env.cpuUsage ?? 0).toFixed(1)}%</div>
                    <div className="text-[10px] text-white/40">CPU</div>
                    <div className="h-1 bg-white/10 rounded-full mt-1.5 overflow-hidden">
                      <div className="h-full bg-blue-400 rounded-full" style={{ width: `${Math.min(env.cpuUsage ?? 0, 100)}%` }} />
                    </div>
                  </div>
                  <div className="bg-white/5 rounded-lg p-2.5 text-center">
                    <MemoryStick className="w-4 h-4 text-purple-400 mx-auto mb-1" />
                    <div className="text-lg font-bold text-white">{Math.round(env.ramUsageMb ?? 0)}</div>
                    <div className="text-[10px] text-white/40">RAM MB</div>
                    <div className="h-1 bg-white/10 rounded-full mt-1.5 overflow-hidden">
                      <div className="h-full bg-purple-400 rounded-full" style={{ width: `${Math.min(((env.ramUsageMb ?? 0) / 512) * 100, 100)}%` }} />
                    </div>
                  </div>
                  <div className="bg-white/5 rounded-lg p-2.5 text-center">
                    <Zap className="w-4 h-4 text-yellow-400 mx-auto mb-1" />
                    <div className="text-lg font-bold text-white">{(env.gpuUsage ?? 0).toFixed(1)}%</div>
                    <div className="text-[10px] text-white/40">GPU</div>
                    <div className="h-1 bg-white/10 rounded-full mt-1.5 overflow-hidden">
                      <div className="h-full bg-yellow-400 rounded-full" style={{ width: `${Math.min(env.gpuUsage ?? 0, 100)}%` }} />
                    </div>
                  </div>
                </div>
              )}
            ...
            {env?.previewUrl && isRunning && (
              <div className="bg-white/5 rounded-lg p-3 border border-white/10 flex items-center gap-2">
                ...
                <span className="text-xs text-white/60 flex-1">Preview URL</span>
            ...
            {isRunning && timeUntilShutdown() && (
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs font-semibold text-yellow-400">Auto-Shutdown בעוד {timeUntilShutdown()}</div>
                  <div className="text-[11px] text-white/40">הסביבה תכבה אוטומטית בחוסר פעילות</div>
                </div>
              </div>
            )}
            ...
            {env?.metadata && Object.keys(env.metadata).length > 0 && (
              <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                <div className="text-xs font-semibold text-white/60 mb-2">מידע נוסף</div>
            ...
            {!env && (
              <div className="bg-white/5 rounded-lg p-6 border border-dashed border-white/20 text-center">
                ...
                <p className="text-white/25 text-xs mt-1">לחץ "הפעל" כדי ליצור סביבה חדשה</p>
              </div>
            )}

            <button onClick={fetchEnv} className="w-full text-xs text-white/30 hover:text-white/60 py-1 transition-colors flex items-center justify-center gap-1">
