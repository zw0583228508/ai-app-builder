...
export function CostPanel({ projectId }: CostPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "optimize">(
    "overview",
  );
  ...
  const formatUsd = (v: number) =>
    v < 0.01 ? `$${(v * 100).toFixed(2)}¢` : `$${v.toFixed(2)}`;
      ...
      <div className="px-4 py-3 border-b border-white/10 bg-gradient-to-r from-yellow-500/10 to-transparent">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">
            Cost Engine — מנוע עלויות
          </h2>
        </div>
        <p className="text-xs text-white/40 mt-0.5">
          מעקב עלויות per-resource + AI Optimization Agent
        </p>
      </div>
            ...
            {!optimization && !loadingOptimize && (
              <div className="flex flex-col items-center justify-center h-32 gap-3">
                ...
                <p className="text-white/30 text-sm text-center">
                  לחץ לניתוח עלויות והמלצות חיסכון
                </p>
              </div>
            )}
              ...
              <>
                ...
                <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-green-400">
                    {optimization.savings_percent?.toFixed(0)}% חיסכון
                  </div>
                  <div className="text-xs text-white/50 mt-1">
                    מ-${optimization.current_monthly_estimate_usd?.toFixed(2)}{" "}
                    ל-${optimization.optimized_monthly_estimate_usd?.toFixed(2)}
                    /mo
                  </div>
                </div>
                ...
                {optimization.predicted_growth_cost && (
                  <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {Object.entries(optimization.predicted_growth_cost).map(
                        ([period, cost]) => (
                ...
                <div className="space-y-2">
                  ...
                  {optimization.recommendations
                    ?.sort(
                      (a, b) =>
                        b.savings_usd_per_month - a.savings_usd_per_month,
                    )
                    .map((rec, i) => (
                      <div
                        key={i}
                        className={cn(
                          "bg-white/5 rounded-lg p-3 border",
                          PRIORITY_COLORS[rec.priority] ?? "border-white/10",
                        )}
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span
                            className={cn(
                              ...
                              EFFORT_COLORS[rec.effort] ?? "",
                            )}
                          >
                            {rec.effort}
                          </span>
                          <span className="text-xs text-green-400 font-semibold">
                            ${rec.savings_usd_per_month}/mo
                          </span>
                        </div>
