...
export function UsagePanel({ projectId }: UsagePanelProps) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ summary: UsageSummary; daily: Record<string, number> }>(
    ...
    ? [
        {
          ...
          label: "הודעות AI",
          value: formatNumber(summary.aiMessages),
          sub: "ב-30 ימים האחרונים",
        },
        {
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2">
              {stats.map((stat, i) => (
                <div
                  key={i}
                  className="p-3 rounded-xl bg-slate-800/60 border border-white/5"
                >
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <div
                        ...
                        title={`${day}: ${formatNumber(val)} טוקנים`}
                      >
                        <div
            ...
            <div className="p-3 rounded-xl bg-slate-800/40 border border-white/5">
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  {
                    label: "הודעות AI ליום",
                    value: `${Math.min(summary?.aiMessages ?? 0, 50)} מתוך 50`,
                    pct: Math.min(((summary?.aiMessages ?? 0) / 50) * 100, 100),
                  },
                ].map((item) => (
                  <div key={item.label}>
