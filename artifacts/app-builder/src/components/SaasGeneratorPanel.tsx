...
export function SaasGeneratorPanel({
  projectId,
  onCodeGenerated,
}: SaasGeneratorPanelProps) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setProgress((prev) => [
        ...prev,
        `❌ שגיאה: ${e instanceof Error ? e.message : "נסה שנית"}`,
      ]);
    } finally {
      setGenerating(false);
    }
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-white/10 bg-gradient-to-r from-[hsl(191,90%,42%)]/15 via-purple-500/10 to-transparent">
        ...
        <p className="text-xs text-white/40 mt-0.5">
          מייצר אפליקציית SaaS מלאה בלחיצה אחת
        </p>
      </div>
      ...
      <div className="px-4 py-2.5 border-b border-white/10 flex flex-wrap gap-1.5">
        {SAAS_FEATURES.map((f, i) => (
          <div
            key={i}
            className={cn(
              "flex items-center gap-1 px-2 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-medium",
              f.color,
            )}
          >
            {f.icon}
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="bg-white/3 border border-white/10 rounded-lg p-3">
            <div className="text-xs text-white/40 leading-relaxed">
              ✨ מייצר קוד מלא עם: דשבורד, Auth, Admin, Billing עם 3 תוכניות
              מחיר, Analytics, ו-Settings מלא. הקוד ייכתב ישירות לפרויקט ויהיה
              מוכן לשימוש.
            </div>
          </div>
        </div>
      ...
      {generating && (
        <div className="flex-1 flex flex-col p-4 overflow-hidden">
            ...
            <div>
              ...
              {codeChars > 0 && (
                <div className="text-xs text-white/40">
                  {codeChars.toLocaleString()} תווי קוד נוצרו
                </div>
              )}
            </div>
          </div>
          ...
          <div className="h-1.5 bg-white/10 rounded-full mb-4 overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[hsl(191,90%,42%)] to-purple-500 rounded-full animate-pulse"
              style={{ width: "60%" }}
            />
          </div>
          ...
          <div
            ref={progressRef}
            className="flex-1 overflow-y-auto bg-black/20 rounded-lg p-3 text-xs text-white/70 font-mono leading-relaxed"
          >
            {progress.map((line, i) => (
              <div key={i}>{line}</div>
            ))}
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-6">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-white/50 mb-4">
              האפליקציה מוכנה עם כל הפיצ'רים: Auth, Dashboard, Admin, Billing,
              Analytics
            </div>
          </div>
          <div className="flex flex-wrap gap-2 justify-center">
            {SAAS_FEATURES.map((f, i) => (
              <div
                key={i}
                className={cn(
                  "flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-medium",
                  f.color,
                )}
              >
