...
import {
  ArrowRight, Github, Rocket, Loader2, Check, ExternalLink,
  ChevronDown, Pencil, Zap, Circle, GitBranch, Globe
} from "lucide-react";
import { ProjectWithMessages, useUpdateProject } from "@workspace/api-client-react";
...
export function ProjectHeader({ project }: ProjectHeaderProps) {
  ...
  const handleRename = async () => {
    ...
    if (!trimmed || trimmed === project.title) { setIsRenaming(false); return; }
    setIsRenaming(false);
    await updateProject.mutateAsync({ id: project.id, data: { title: trimmed } });
  ...
  const sendChatAction = (action: string) => {
    setShowGithubMenu(false);
    setShowDeployMenu(false);
    window.dispatchEvent(new CustomEvent("builder-header-action", { detail: { action } }));
  };
    ...
    >
      ...
      <div className="flex items-center gap-1.5 min-w-0 flex-1">
        {isRenaming ? (
          ...
          <button
            onClick={() => { setRenameValue(project.title); setIsRenaming(true); }}
        ...
        {/* Status pill */}
        <div className={cn("flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-white/[0.03] border border-white/[0.06] shrink-0", statusColor)}>
          {isBuilding
            ? <Loader2 className="w-2.5 h-2.5 animate-spin" />
            : <Circle className={cn("w-1.5 h-1.5 fill-current", hasDeployment ? "text-emerald-400" : hasGithub ? "text-sky-400" : "text-slate-600")} />
          }
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <div className="relative" ref={githubMenuRef}>
          ...
          >
            ...
            <ChevronDown className={cn("w-3 h-3 transition-transform opacity-50", showGithubMenu && "rotate-180")} />
          </button>

          {showGithubMenu && (
            <div className="absolute top-full mt-1.5 left-0 w-52 bg-[#16161f] border border-white/[0.08] rounded-xl shadow-xl shadow-black/50 overflow-hidden z-50" dir="rtl">
              {hasGithub ? (
                <>
                  <a
        ...
        <div className="relative" ref={deployMenuRef}>
          <button
            onClick={() => hasDeployment ? setShowDeployMenu(v => !v) : sendChatAction("deploy")}
            className={cn(
          ...
          >
            {hasDeployment
              ? <><Globe className="w-3.5 h-3.5" /><span className="hidden sm:inline">פורסם</span><ChevronDown className="w-3 h-3 opacity-60" /></>
              : <><Rocket className="w-3.5 h-3.5" /><span className="hidden sm:inline">פרסם</span></>
            }
          </button>
