...
export function TeamPanel() {
  ...
  const {
    ...
    queryKey: ["teams"],
    queryFn: () =>
      apiFetch<{ owned: Team[]; member: Array<{ team: Team; role: string }> }>(
        "/api/teams",
      ),
    retry: false,
    throwOnError: false,
  ...
  const { data: teamDetail } = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ team: Team; members: TeamMember[] }>(
  ...
  const joinMutation = useMutation({
    mutationFn: (code: string) =>
      apiFetch<{ team: Team }>("/api/teams/join", {
        method: "POST",
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        ...
        {/* Error */}
        {error && (
          <div className="px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/25 text-xs text-red-300">
            {error}
          </div>
        )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-right">
                    ...
                    <p className="text-xs text-slate-400">
                      {teamsData?.owned.some((t) => t.id === team.id)
                        ? "בעלים"
                        : "חבר"}
                    </p>
                  </div>
                  ...
                  <div className="mt-2 mr-4 p-3 rounded-xl bg-slate-900/80 border border-white/5 space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800 border border-white/8">
                        <code className="text-xs text-cyan-300 font-mono flex-1 tracking-widest">
                          {teamDetail.team.inviteCode}
                        </code>
                        <button
                          onClick={() =>
                            copyInviteCode(teamDetail.team.inviteCode)
                          }
                    ...
                    <div>
                      <p className="text-xs text-slate-400 mb-1.5">
                        חברים ({teamDetail.members.length})
                      </p>
                      <div className="space-y-1.5">
                        {teamDetail.members.map((member) => (
                          ...
                          >
                            ...
                            >
                              {member.role === "owner" && (
                                <Crown className="w-2.5 h-2.5 inline ml-0.5" />
                              )}
                              {roleLabel(member.role)}
                            </span>
                            {member.role !== "owner" &&
                              teamDetail.team.ownerId && (
                                <button
                                  onClick={() =>
