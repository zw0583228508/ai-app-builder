...
import {
  Sparkles, ChevronRight, ChevronLeft, Rocket, Code2, Store, Palette,
  Check, ArrowRight,
} from "lucide-react";
...
interface OnboardingPanelProps {
  onComplete: (data: { userMode: UserMode; buildMode: BuildMode; projectType: string }) => void;
  onSkip?: () => void;
}

const MODES: { id: UserMode; label: string; description: string; icon: React.ReactNode; color: string; bg: string; border: string }[] = [
  {
    id: "entrepreneur",
    label: "יזם",
...
export default function OnboardingPanel({ onComplete, onSkip }: OnboardingPanelProps) {
  const [step, setStep] = useState<1 | 2 | 3>(1);
  const [selectedMode, setSelectedMode] = useState<UserMode | null>(null);
  const [selectedBuildMode, setSelectedBuildMode] = useState<BuildMode>("quick");
  const [selectedType, setSelectedType] = useState<string>("website");

  function handleFinish() {
    if (!selectedMode) return;
    onComplete({ userMode: selectedMode, buildMode: selectedBuildMode, projectType: selectedType });
  }
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="flex items-center gap-2 mb-8">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              ...
              {s < 3 && (
                <div className={cn("h-px w-8 flex-1", step > s ? "bg-violet-500" : "bg-white/10")} />
              )}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div>
              <h1 className="text-2xl font-bold text-white mb-1">ברוך הבא! 👋</h1>
            ...
            <div className="grid grid-cols-2 gap-3">
              {MODES.map((mode) => (
                ...
                >
                  <div className={cn("mb-2", selectedMode === mode.id ? mode.color : "text-white/50")}>
                    {mode.icon}
                  </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              <h1 className="text-2xl font-bold text-white mb-1">מה תרצה לבנות?</h1>
              <p className="text-white/50">בחר את סוג הפרויקט — ניתן לשנות מאוחר יותר</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {PROJECT_TYPES.map((t) => (
                <button
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div>
              <h1 className="text-2xl font-bold text-white mb-1">איך להתחיל?</h1>
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-3 mb-1">
                  <Sparkles className={cn("w-5 h-5", selectedBuildMode === "quick" ? "text-violet-400" : "text-white/40")} />
                  <span className={cn("font-semibold", selectedBuildMode === "quick" ? "text-violet-300" : "text-white")}>
                    בנייה מהירה
                  </span>
                </div>
                <p className="text-sm text-white/50 mr-8">כתוב מה אתה רוצה — הAI יתחיל לבנות מיד</p>
              </button>
              <button
              ...
              >
                <div className="flex items-center gap-3 mb-1">
                  <ArrowRight className={cn("w-5 h-5", selectedBuildMode === "guided" ? "text-blue-400" : "text-white/40")} />
                  <span className={cn("font-semibold", selectedBuildMode === "guided" ? "text-blue-300" : "text-white")}>
                    בנייה מודרכת
                  </span>
                </div>
                <p className="text-sm text-white/50 mr-8">הAI ישאל כמה שאלות, ייצר מפרט, ואז יבנה</p>
              </button>
            </div>
            <div className="flex gap-3">
              <button
