    ...
    return (
      <div>
        ...
        >
          ...
          <span className="text-xs text-foreground/80 truncate">{node.name}</span>
        </button>
        {open && Object.values(node.children).sort((a, b) => {
          const aIsDir = !a.file && Object.keys(a.children).length > 0;
          const bIsDir = !b.file && Object.keys(b.children).length > 0;
          if (aIsDir !== bIsDir) return aIsDir ? -1 : 1;
          return a.name.localeCompare(b.name);
        }).map((child) => (
          <TreeNodeItem key={child.path} node={child} depth={depth + 1} selectedId={selectedId} onSelect={onSelect} onDelete={onDelete} />
        ))}
      </div>
    );
  }
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isSelected ? "bg-primary/15 text-primary" : "hover:bg-white/5 text-foreground/70 hover:text-foreground"
      )}
    ...
    >
      ...
      {f.isEntrypoint && (
        <span className="text-[9px] px-1 py-0.5 bg-cyan-500/20 text-cyan-400 rounded shrink-0">entry</span>
      )}
      {onDelete && (
        <span
          role="button"
...
export function FileTree({ files, selectedId, onSelect, onDelete, onNewFile }: FileTreeProps) {
  ...
  return (
    <div className="flex flex-col h-full" dir="ltr">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/5">
        <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">קבצים</span>
        {onNewFile && (
          <button
            onClick={onNewFile}
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {files.length === 0 ? (
          <div className="px-3 py-4 text-xs text-muted-foreground text-center">אין קבצים עדיין</div>
        ) : (
          sorted.map((node) => (
            <TreeNodeItem key={node.path} node={node} depth={0} selectedId={selectedId} onSelect={onSelect} onDelete={onDelete} />
          ))
        )}
      </div>
    </div>
