...
export function ErrorsPanel({ projectId }: ErrorsPanelProps) {
  ...
  const clearMutation = useMutation({
    mutationFn: () =>
      apiFetch(`/api/projects/${projectId}/errors`, { method: "DELETE" }),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["app-errors", projectId] }),
  });
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-slate-500 gap-2">
            ...
            <p className="text-xs text-center max-w-xs text-slate-600">
              שגיאות JS מהאפליקציה הפרוסה יופיעו כאן אוטומטית לאחר פרסום
            </p>
          </div>
        ) : (
