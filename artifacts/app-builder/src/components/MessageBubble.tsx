...
export function extractCodeAndText(content: string): {
    ...
    if (fenceStart !== -1) {
      ...
      if (closingMatch !== -1) {
        ...
        const closingEnd =
          afterTag +
          closingMatch +
          content.slice(afterTag + closingMatch).indexOf("\n```") +
          4;
        const text = (
          content.slice(0, fenceStart) + content.slice(closingEnd)
        ).trim();
  ...
  if (genericFence !== -1) {
    ...
    if (closingFence !== -1) {
      ...
      const text = (
        content.slice(0, genericFence) + content.slice(closingFence + 4)
      ).trim();
          ...
          <div className="mt-2">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span
                  className="text-slate-600 mr-auto text-[10px]"
                  style={{ fontFamily: HE }}
                >
                  מוצג בצד ימין
                </span>
                  ...
                  <div className="relative rounded-xl border border-white/[0.08] bg-[#0a0a0f] overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.05] bg-[#0d0d15]">
                      <span className="text-[11px] text-slate-600 font-mono">
                        {codeLang === "html" || codeLang === null
                          ? "index.html"
                          : codeLang === "jsx" || codeLang === "tsx"
                            ? `App.${codeLang}`
                            : codeLang === "css"
                              ? "style.css"
                              : codeLang === "js" || codeLang === "ts"
                                ? `index.${codeLang}`
                                : `output.${codeLang}`}
                      </span>
                      <button
                    ...
                    <pre
                      className="p-4 text-[11px] text-slate-400 font-mono leading-relaxed overflow-x-auto max-h-80 overflow-y-auto"
                      dir="ltr"
                    >
