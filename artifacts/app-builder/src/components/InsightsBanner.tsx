...
export function InsightsBanner({
  ...
  if (loading || dismissed || !data || data.suggestions.length === 0)
    return null;
          ...
          return (
            <div key={i} className="px-3 py-2">
              ...
              >
                <Icon
                  size={13}
                  className={`mt-0.5 shrink-0 ${s.priority === "high" ? "text-red-400" : s.priority === "medium" ? "text-yellow-400" : "text-blue-400"}`}
                />
                <span className="flex-1 text-xs text-slate-300 text-right leading-snug">
                  {s.problem}
                </span>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded border shrink-0 ${priorityColor[s.priority]}`}
                >
              ...
              {isOpen && (
                <div className="mt-2 mr-5 space-y-1.5">
                  <div className="text-[11px] text-slate-400">
                    <span className="text-slate-300 font-medium">פתרון: </span>
                    {s.fix}
                  </div>
                  <div className="text-[11px] text-slate-400">
                    <span className="text-slate-300 font-medium">השפעה: </span>
                    {s.impact}
                  </div>
                  {onFixRequest && (
                    <button
