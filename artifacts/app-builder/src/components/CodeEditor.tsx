...
export function CodeEditor({
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${protocol}//${host}/api/collab?projectId=${projectId}&name=משתמש`,
    );
    wsRef.current = ws;
    ...
    ws.onmessage = (evt) => {
      try {
        ...
        if (msg.type === "presence") {
          ...
          setRemoteCursors(
            (msg.cursors ?? []).filter((c) =>
              filePath ? c.file === filePath : true,
            ),
          );
        }
  ...
  const sendCursor = useCallback(
    (line: number, col: number) => {
      if (wsRef.current?.readyState === WebSocket.OPEN) {
        wsRef.current.send(
          JSON.stringify({ type: "cursor", line, col, file: filePath ?? "" }),
        );
      }
    },
    [filePath],
