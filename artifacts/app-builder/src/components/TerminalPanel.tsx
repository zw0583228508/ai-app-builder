...
import { Loader2, Wifi, WifiOff, RotateCcw, Maximize2, Search, X } from "lucide-react";
...
export function TerminalPanel() {
  ...
  function handleSearch(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (searchRef.current) {
      if (val) searchRef.current.findNext(val, { caseSensitive: false, regex: false });
    }
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0c] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border/30 bg-[#0d0d10] shrink-0">
        <div className="flex items-center gap-2">
          <div className={cn(
            ...
            connState === "connected" ? "bg-green-400 shadow-sm shadow-green-400/50" :
            connState === "connecting" ? "bg-yellow-400 animate-pulse" :
            "bg-red-400"
          )} />
          <span className="text-xs text-muted-foreground" style={{ fontFamily: HE }}>
            {connState === "connected" ? "מחובר" :
             connState === "connecting" ? "מתחבר..." :
             connState === "error" ? "שגיאת חיבור" : "מנותק"}
          </span>
          {connState === "connecting" && <Loader2 className="w-3 h-3 animate-spin text-muted-foreground" />}
        </div>
            ...
            <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-md px-2 py-0.5 mr-1">
              <input
                ...
                onKeyDown={e => {
                  ...
                  if (e.key === "Enter") searchRef.current?.findNext(searchTerm);
                }}
              />
      ...
      {/* Terminal container */}
      <div className="flex-1 overflow-hidden p-1" onClick={() => termRef.current?.focus()}>
