...
export function StoragePanel({ projectId }: StoragePanelProps) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ objects: StorageObject[] }>(
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      apiFetch(`/api/projects/${projectId}/storage/${id}`, {
        method: "DELETE",
      }),
    onSuccess: () =>
    ...
    >
      ...
      <div className="mx-4 mt-3 mb-1 px-3 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg">
        <p className="text-xs text-slate-400 leading-relaxed">
          העלה תמונות, מסמכים ומדיה לשימוש בפרויקט. כל קובץ מקבל URL ציבורי
          שניתן לשלב ישירות בקוד.
        </p>
      </div>
