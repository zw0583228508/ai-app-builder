...
import { Brain, Sparkles, Save, RefreshCw, ChevronDown, ChevronUp, X, Plus, Zap, Check } from "lucide-react";
...
const INDUSTRY_OPTIONS = [
  "SaaS", "eCommerce", "FinTech", "HealthTech", "EdTech", "Media",
  "Gaming", "Real Estate", "Marketing", "B2B", "B2C", "Startup",
];

const STACK_OPTIONS = [
  "React", "Vue", "Svelte", "Angular", "Next.js", "Remix",
  "Node.js", "Python", "TypeScript", "Tailwind CSS", "shadcn/ui",
];
...
export function BusinessMemoryPanel() {
  ...
  async function extract() {
    ...
    try {
      ...
      const d = await res.json();
      if (d.dna) { setDna(d.dna); setHasChanges(false); }
    } finally {
      setExtracting(false);
  ...
  function removeGoal(i: number) {
    update("businessGoals", (dna.businessGoals ?? []).filter((_, idx) => idx !== i));
  }

  function toggleIndustry(ind: string) {
    const current = dna.industryFocus ?? [];
    if (current.includes(ind)) {
      update("industryFocus", current.filter(x => x !== ind));
    } else {
  ...
  function toggleStack(s: string) {
    ...
    if (current.includes(s)) {
      update("frameworks", current.filter(x => x !== s));
    } else {
    ...
    return (
      <div className="border border-border/40 rounded-xl overflow-hidden">
        ...
        >
          ...
          <span className="flex-1 text-sm font-semibold text-foreground" style={{ fontFamily: HE }}>{title}</span>
    ...
    return (
      <div className="flex items-center justify-center h-48">
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-sm" style={{ fontFamily: HE }}>טוען זיכרון עסקי...</span>
        </div>
      </div>
    );
  }

  const memoryActive = !!(dna.skillLevel || dna.preferredStack || dna.uiStyle ||
    (dna.industryFocus?.length ?? 0) > 0 || (dna.businessGoals?.length ?? 0) > 0);
  ...
  return (
    <div className="flex flex-col h-full" dir="rtl">
      ...
      <div className="p-4 border-b border-border/40 shrink-0">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-bold text-foreground" style={{ fontFamily: HE }}>זיכרון עסקי</h2>
            <p className="text-xs text-muted-foreground" style={{ fontFamily: HE }}>ההעדפות שלך מוזנות לכל build אוטומטית</p>
          </div>
          <div className={cn(
            ...
            <span className={cn("w-1.5 h-1.5 rounded-full", memoryActive ? "bg-green-400 animate-pulse" : "bg-muted-foreground")} />
        ...
        {(dna.totalProjects ?? 0) > 0 && (
          <div className="flex items-center gap-4 mt-3 px-1">
            <div className="text-center">
              <p className="text-lg font-bold text-primary tabular-nums">{dna.totalProjects ?? 0}</p>
              <p className="text-[10px] text-muted-foreground" style={{ fontFamily: HE }}>פרויקטים</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-bold text-foreground tabular-nums">{Math.round((dna.totalTokensUsed ?? 0) / 1000)}K</p>
              <p className="text-[10px] text-muted-foreground" style={{ fontFamily: HE }}>טוקנים</p>
            </div>
            {dna.updatedAt && (
              <div className="text-center flex-1">
                <p className="text-xs text-muted-foreground" style={{ fontFamily: HE }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        >
          {extracting
            ? <><Sparkles className="w-4 h-4 animate-spin" /><span>מחלץ מההיסטוריה...</span></>
            : <><Zap className="w-4 h-4" /><span>חלץ DNA אוטומטית מהפרויקטים שלי</span></>
          }
        </button>
        ...
        <Section id="skills" title="כישורים ורמה" icon="🧠">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>רמת ניסיון</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>טכנולוגיות מועדפות</label>
        ...
        <Section id="style" title="העדפות עיצוב ו-Stack" icon="🎨">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>סגנון UI מועדף</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>Stack מועדף לפרויקטים חדשים</label>
            <select
              value={dna.preferredStack ?? ""}
              onChange={e => update("preferredStack", e.target.value || undefined)}
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>העדפת deploy</label>
        ...
        <Section id="business" title="הקשר עסקי ומטרות" icon="🏢">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>תחומי פעילות</label>
            <div className="flex flex-wrap gap-1.5">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>יעדים עסקיים ({dna.businessGoals?.length ?? 0}/5)</label>
            {(dna.businessGoals ?? []).map((goal, i) => (
              <div key={i} className="flex items-center gap-2 mb-1.5">
                <div className="flex-1 bg-muted/30 border border-border/30 rounded-lg px-3 py-1.5 text-xs text-foreground" style={{ fontFamily: HE }}>
                  {goal}
                </div>
                <button onClick={() => removeGoal(i)} className="text-muted-foreground hover:text-red-400 transition-colors p-1">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-2 block" style={{ fontFamily: HE }}>רגישות לעלות</label>
            <div className="flex gap-2">
              {[
      ...
      <div className="p-4 border-t border-border/40 shrink-0">
        ...
        >
          {saving
            ? <><RefreshCw className="w-4 h-4 animate-spin" /><span>שומר...</span></>
            : saved
              ? <><Check className="w-4 h-4" /><span>נשמר!</span></>
              : <><Save className="w-4 h-4" /><span>שמור זיכרון עסקי</span></>
          }
        </button>
        <p className="text-[10px] text-muted-foreground/50 text-center mt-1.5" style={{ fontFamily: HE }}>
          {memoryActive ? "הזיכרון מוזן לכל build אוטומטית" : "מלא פרטים כדי לשפר את איכות ה-build"}
        </p>
      </div>
    </div>
  );
