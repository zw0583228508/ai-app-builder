...
export function useProjectFiles(projectId: number | undefined) {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const { projectId: pid } = (e as CustomEvent<{ projectId: number }>)
        .detail;
      if (pid === projectId) {
...
export function useUpdateProjectFile(projectId: number) {
  ...
  return useMutation({
    mutationFn: (vars: {
      ...
      apiFetch<{ file: ProjectFile }>(
        `/api/projects/${projectId}/files/${vars.fileId}`,
        {
          method: "PUT",
          body: JSON.stringify({
...
export function useDeleteProjectFile(projectId: number) {
  ...
  return useMutation({
    mutationFn: (fileId: number) =>
      apiFetch<{ ok: boolean }>(`/api/projects/${projectId}/files/${fileId}`, {
        method: "DELETE",
      }),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["project-files", projectId] }),
