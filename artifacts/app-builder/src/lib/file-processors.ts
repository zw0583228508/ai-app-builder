...
export async function processZipFile(file: File): Promise<ProcessedFile> {
  ...
  const sortedFiles = Object.entries(zip.files).sort(([a], [b]) => a.localeCompare(b));
  ...
  for (const [path, entry] of sortedFiles) {
    if (entry.dir) {
      ...
      const isTextLike = /\.(txt|html|css|js|ts|jsx|tsx|json|md|yaml|yml|xml|sql|py|rb|php|sh|env|gitignore|dockerfile|toml|ini|cfg|conf|csv|scss|sass|less|vue|svelte|kt|swift|go|rs|c|cpp|h|java|cs)$/i.test(path);
      if (isTextLike) {
        try {
  ...
  lines.push("```\n");
  lines.push(`**${sortedFiles.filter(([, e]) => !e.dir).length} קבצים** | **${folders.size} תיקיות**\n`);
...
function getFileIcon(ext: string): string {
  const icons: Record<string, string> = {
    html: "🌐", css: "🎨", js: "⚡", ts: "🔷", json: "📋",
    md: "📝", txt: "📄", py: "🐍", rb: "💎", php: "🐘",
    sql: "🗄️", sh: "💻", env: "🔒", jpg: "🖼️", jpeg: "🖼️",
    png: "🖼️", gif: "🖼️", svg: "🎭", pdf: "📕", zip: "📦",
    mp3: "🎵", wav: "🎵", mp4: "🎬", csv: "📊", xlsx: "📊",
    docx: "📄", yaml: "⚙️", yml: "⚙️", toml: "⚙️", go: "🐹",
    rs: "🦀", kt: "🎯", swift: "🍎", c: "⚙️", cpp: "⚙️", java: "☕",
  };
  return icons[ext] || "📄";
}
